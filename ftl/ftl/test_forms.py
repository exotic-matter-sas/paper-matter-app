...
from ftests.tools.setup_helpers import setup_org, setup_user, setup_2fa_static_device, setup_2fa_fido2_device, \
    setup_2fa_totp_device
from ftests.tools import test_values as tv
from .forms import FTLUserCreationForm
from ftl.otp_plugins.otp_ftl.forms import StaticDeviceCheckForm, TOTPDeviceCheckForm, StaticDeviceForm, TOTPDeviceForm
...
class StaticDeviceCheckFormTests(TestCase):
    def setUp(self):
        ...
        self.user_2 = setup_user(org=self.org_1, email=tv.USER2_EMAIL, password=tv.USER2_PASS)
        self.user_3 = setup_user(org=self.org_2, email=tv.USER3_EMAIL, password=tv.USER3_PASS)
...
class StaticDeviceFormTests(TestCase):
    ...
    def test_form_save_proper_model(self, random_token_mock):
        ...
        self.assertCountEqual(
            StaticToken.objects.filter(device_id=expected_model.id).values_list("token", flat=True),
            StaticToken.objects.filter(device_id=saved_model.id).values_list("token", flat=True)
        )
...
class TOTPDeviceFormTests(TestCase):
    def setUp(self):
        self.org_1 = setup_org()
        self.user_1 = setup_user(org=self.org_1)
        self.user_1_totp_device_1 = setup_2fa_totp_device(self.user_1, "user1 totp 1", confirmed=False)
    ...
    def test_form_save_proper_model(self, random_hex_mock):
        ...
        expected_model = setup_2fa_totp_device(self.user_1, tv.TOTP_DEVICE_NAME, secret_key=tv.TOTP_DEVICE_SECRET_KEY)
