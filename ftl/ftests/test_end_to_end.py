...
class UserSetupAll2FA(LoginPage, AccountPages):
    ...
    @patch.object(TOTP, 'time', totp_time_property)
    @skipIf(DEV_MODE and not NODE_SERVER_RUNNING, "Node not running, this test can't be run")
    def test_reduced_session_timeout_during_2fa_check(self):
        totp_time_setter.side_effect = mocked_totp_time_setter
        ...
        delta = datetime.fromtimestamp(cookie['expiry'], timezone.utc) - now
        # Must be lesser than 10 mins
        ...
        self.assertGreaterEqual(delta, timedelta(weeks=2))
