...
class FTLOCRGoogleVisionAsync(FTLOCRBase):
    ...
    def __init__(
        ...
        ).get_bucket(gcs_bucket_name)
        self.client = vision_v1.ImageAnnotatorClient(credentials=credentials)
        self.supported_storages = [FTLStorages.GCS]

        self.mime_type = "application/pdf"
        self.batch_size = (
            10  # How many pages should be grouped into each json output file
        )
        self.feature = vision.types.Feature(
    ...
    def _extract_text(self, ftl_doc_binary):
        ...
        storage_uri_ocr_target = (
            f"gs://{self.gcs_bucket_name}/{ftl_doc_binary.name}.ocr."
        )
