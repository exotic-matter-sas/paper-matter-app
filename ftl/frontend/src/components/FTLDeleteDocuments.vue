          ...
          .then(res => {
            ...
            if (res.length === nb) {
              ...
              this.mixinAlertWarning(
                this.$tc('No document could be deleted. | One document deleted successfully ({remain} couldn\'t). | {n} documents deleted successfully ({remain} couldn\'t).',
                  res.length, {remain: nb - res.length}));
            }
          });
      }
    }
