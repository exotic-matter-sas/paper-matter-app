...
<template>
  ...
  >
    <span slot="modal-title">{{
      $t('Deletion of folder "{0}" and its contents', [this.folder.name])
    }}</span>
    <b-container>
      <b-form-group
        ...
        :label="
          $t(
            'Please confirm that you want to delete the folder and everything inside. This action is not reversible.'
          )
        "
  ...
  methods: {
    deleteFolder: function (bvModalEvt) {
      ...
      axios
        ...
        .catch((error) =>
          this.mixinAlert(this.$t("Unable to delete folder"), true)
        )
        .then(() => {
          this.deleting = false;
        });
