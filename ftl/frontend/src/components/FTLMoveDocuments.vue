  ...
  <b-modal :id="id">
    <template slot="modal-title">
      <span>
        {{
          $tc(
            '| Move "{documentTitle}" to: | Move {n} documents to:',
            docs.length,
    ...
    <template slot="modal-footer">
      ...
      >
        <span
          v-if="selectedMoveTargetFolder"
          :title="selectedMoveTargetFolder.name"
        >
      ...
      <b-button
        variant="primary"
        @click.prevent="moveDocument"
        :disabled="!selectedMoveTargetFolder"
      >
        {{ $t("OK") }}
      </b-button>
    </template>
  ...
  methods: {
    moveDocument: function () {
      ...
      for (const doc of this.docs) {
        ...
        promisesUpdate.push(
          axios.patch("/app/api/v1/documents/" + doc.pid, body, {
            ...axiosConfig,
            docPid: doc.pid,
          })
        );
        ...
        .then((responses) => {
          ...
          if (responses.length === nb) {
            ...
            this.mixinAlertWarning(
              this.$tc(
                "No document could be moved. | One document moved successfully ({remain} couldn't). | {n} documents moved successfully ({remain} couldn't).",
                responses.length,
                { remain: nb - responses.length }
              )
            );
          }
        });
