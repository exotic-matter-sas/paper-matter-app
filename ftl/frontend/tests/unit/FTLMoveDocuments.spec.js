...
const mockedSelectedMoveTargetFolderResponse = {
  id: tv.FOLDER_PROPS.id,
...
describe("Component methods error handling", () => {
  ...
  it("moveFolder call mixinAlert in case of error", async () => {
    ...
    expect(mockedMixinAlert.mock.calls[0][0]).toContain(
...
describe("Event emitted by component", () => {
  ...
  beforeEach(() => {
    axios.patch.mockResolvedValue(mockedMoveDocumentResponse);
    mockedSelectedMoveTargetFolder.mockReturnValue(
      mockedSelectedMoveTargetFolderResponse
    );
